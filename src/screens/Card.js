...
function Card({ data }) {
  ...
  const mapp = filteredData.map((one) => {
    const imageSliced = one.images.slice(0, 4);
            ...
            <div className="p-5">
              <a href="#">
                <h5 className="mb-2 text-2xl font-bold tracking-tight card-text-flowbite">
                  {one.propertyType ? one.propertyType : "House"}
                </h5>
              </a>
  ...
  return <div className="flex items-center">{mapp}</div>;
}
